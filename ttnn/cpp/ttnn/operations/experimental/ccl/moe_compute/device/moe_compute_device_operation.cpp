        ...
        if (args.path == MoEComputePath::FullLocal) {
            ...
            for (const auto& placement : input_topology.placements()) {
                TT_FATAL(
                    ...
                    "path=FullLocal requires a fully replicated logical-token input topology; local expert "
                    "partials are reduced explicitly by the caller");
            }
        } else {
            TT_FATAL(
...
MoEComputeDeviceOperation::topology_return_value_t MoEComputeDeviceOperation::compute_output_topologies(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
    if (args.path != MoEComputePath::FullLocal) {
        // Preserve the pre-existing topology inference for ComputeOnly and FullCcl.
        return {};
    }

    // FullLocal deliberately leaves one full-width additive partial at each mesh
    // coordinate.  It has no tensor-dimension sharding: the fact that values differ
    // between coordinates is a reduction-state invariant tracked by the caller, not a
    // Shard placement.  In particular, do not let the expert-sharded weight inputs make
    // the generic multi-input inference label H (dim 2) as expert-sharded.
    const auto& local_partial_topology = tensor_args.tilize_input_tensor.tensor_topology();
    return topology_return_value_t(6, local_partial_topology);
}
