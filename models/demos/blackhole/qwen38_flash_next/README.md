...
| path | measured | notes |
|---|---|---|
| prompt prefill | 410 tok/s; 750-790 tok/s with `--long-chunks`; 3,003 tok/s with `--prefill-slab 2048` | 2026-09-26 (block-shared attention + one-pass combine + three-ring expert stream + GDN row programs defaults: 31,716 tokens in 10.56 s; the chunk rates 2026-09-25); the slab is tolerance-class against the chunk bodies (`docs/PREFILL.md`: the ms per prompt token, the TTFTs, the one-call expert stream) |
...
| decode with MTP (`--mtp 4`) | greedy 61.3 tok/s on a 560-token chat prompt, 57.6 on a 177-token multi-turn chat, 91.9 code (256-token answers), 95.3 json and 44.7 prose over their whole answers (153 / 196 tokens, EOS honoured; 3.12 / 2.83 / 4.57 / 4.75 / 2.17 tokens per pass, identical to the chain's) against 38.6 plain greedy; `--mtp 3` (2026-09-25) 45.5 / 55.4 / 60.6 / 30.7; `--mtp 5` opt-in (`docs/NUMERICS.md`); sampled 19.8 / 19.9 ms per token (the card profiles, non-thinking / thinking; 33.5 / 33.9 tok/s plain sampled) | answers measured client-side with EOS honoured on the 4-chip p150 line (2026-09-26, the fork runtime built at `5eac9c778edb`, `_ttnncpp.so` `211530d77b2b`; the MoE exchange pipelining added 0.7-1.3 tok/s to every row; the QSA verify-rows glue on the fused programs, on the GDN verify rows and the decode-kernel stack; a 256-token row whose answer runs past 256 tokens, the code answer at 283, is measured to 256; a row over a shorter answer stops at its end, so no post-answer continuation is counted); speculative drafting with exact acceptance for greedy requests and, by default on an `--mtp --sampling` server, sampled ones; the pass costs 48-50 ms greedy whatever it accepts (60-62 before the GDN verify rows ran on the fused row programs; section 6, `docs/NUMERICS.md`); opt-in GDN rows fold (`QWEN38_FUSED=gdn_rows_scan`, 2026-09-26): 62.5 / 57.6 / 95.5 / 97.3 / 43.4 over a 203-token prose answer in the same form, pass 1.0-1.2 ms shorter, sampled 19.7 / 20.9, streams follow plain decode's (`docs/NUMERICS.md`) |
