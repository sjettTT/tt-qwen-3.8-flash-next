...
| decode with MTP (`--mtp 4`) | greedy 60.6 tok/s on a 560-token chat prompt, 57.0 on a 177-token multi-turn chat, 91.0 code (256-token answers), 94.0 json and 44.3 prose over their whole answers (153 / 196 tokens, EOS honoured; 3.12 / 2.83 / 4.57 / 4.75 / 2.17 tokens per pass, identical to the chain's) against 38.6 plain greedy; `--mtp 3` (2026-09-25) 45.5 / 55.4 / 60.6 / 30.7; `--mtp 5` opt-in (`docs/NUMERICS.md`); sampled 50.4 / 49.7 tok/s at 19.9 / 20.1 ms per token (the card profiles, non-thinking / thinking; 42.0 / 41.8 before the GDN rows programs) against 33.5 / 33.9 plain | answers measured client-side with EOS honoured on the 4-chip p150 line (2026-09-26, the QSA verify-rows glue on the fused programs, on the GDN verify rows and the decode-kernel stack; a 256-token row whose answer runs past 256 tokens, the code answer at 283, is measured to 256; a row over a shorter answer stops at its end, so no post-answer continuation is counted); speculative drafting with exact acceptance for greedy requests and, by default on an `--mtp --sampling` server, sampled ones; the pass costs 50-52 ms whatever it accepts (60-62 before the GDN verify rows ran on the fused row programs; section 6, `docs/NUMERICS.md`) |
