...
| decode, one stream | 36.8 tok/s greedy, 36.3 tok/s sampled | position-generic traced decode, 27.2 ms per token, flat with depth; BF8 dense weights, the fused GDN step, the compact expert layout and the MoE post program's one-read routing by default; 9 decode chains run as fused programs, the gated-residual read as two programs with its gathers inside them, the router tail's top-k on one core per token group, and the decode linears read each DRAM bank with two cores by default (2026-09-25, bitwise: `docs/NUMERICS.md`); the sampled step is the on-device sampler after the top-32 candidate row, 27.6 ms per token on both model cards (thinking; instruct with its presence penalty), the host sampler's law (the law gate in `docs/NUMERICS.md`), the default on `--sampling` servers, `--host-sampler` restores host sampling (2026-09-25) |
| decode, 4 / 8 streams (batched-decode lane sweep; the chat server serves one stream) | 28.5 / 23.1 tok/s per user (114 / 185 aggregate) | the fused GDN step's lanes form on B rows, measured with the batched-decode lane body directly: 35.0 / 43.3 ms per step (40.9 / 50.4 with the composite lane body); the one-stream row is unchanged (2026-09-25, component class: `docs/NUMERICS.md`) |
